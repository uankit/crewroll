...
data class ManifestReadResult(
    ...
    fun erase() { contentRoot.fill(0); plaintext.fill(0) }
}
...
object ManifestReader {
    ...
    fun open(
        ...
        } catch (error: Throwable) { plaintext.fill(0); throw error }
        finally { manifestKey.fill(0) }
    }
    ...
    internal fun parse(plaintext: ByteArray): ManifestReadResult {
        ...
        } catch (error: Throwable) { contentRoot.fill(0); throw error }
    }
