...
describe("compiled CrewRoll contracts in the control-plane TypeBox instance", () => {
  it("installs contract formats at composition so Value and TypeCompiler accept valid input", () => {
    ...
    expect(TypeCompiler.Compile(CreateTripBodySchema).Check(validTrip)).toBe(true);
  });
  ...
  it("exposes an idempotent installer with the complete semantic format set", () => {
    ...
    expect(installed.get("date-time")?.("2028-02-30T12:00:00.000Z")).toBe(false);
    expect(installed.get("uuid")?.("018f0d98-76fa-7d1a-b4b4-1f742c2e3120")).toBe(true);
    expect(installed.get("uuid")?.("not-a-uuid")).toBe(false);
    expect(installed.get("uri")?.("https://api.crewroll.example/v1")).toBe(true);
    ...
    expect(installed.get("opaque-source-asset-key")?.("src_01J6D4M4KB8J8G3AZXJ3PZV1Z9XY")).toBe(true);
    expect(installed.get("opaque-source-asset-key")?.("file:///photo.jpg")).toBe(false);
  });
});
